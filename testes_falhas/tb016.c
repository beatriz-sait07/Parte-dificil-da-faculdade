...
void buffer(Lista** l) {
    ...
    while (aux_buf != EOF) {
        if (aux_buf != ' ' && aux_buf != '\t' && aux_buf != '\n') {
            ...
            if (pos_b > 0) { // Verifica se há caracteres no buffer antes de finalizá-lo
                buffer[pos_b] = '\0';
                int isSpecialChar = 0;
                // Começa a verificação aqui!
                for (int i = 0; i < pos_b; i++) {
                    if (buffer[i] >= l[35]->inicio->letra && buffer[i] <= l[44]->fim->letra) {
                        printf("%c\n", buffer[i]);
                        isSpecialChar = 1;
                    }
                }
    ...
    if (pos_b > 0) {
        buffer[pos_b] = '\0';
        int isSpecialChar = 0; // Verifica caracteres especiais no início do buffer
        // Verifica caracteres especiais no final do buffer
        for (int i = 0; i < pos_b; i++) {
            if (buffer[i] >= l[35]->inicio->letra && buffer[i] <= l[44]->inicio->letra) {
                // Se um caractere especial for encontrado, imprima-o como palavra separada
                if(buffer[i] >= l[43]->inicio->letra && buffer[i] <= l[44]->fim->letra){
                    while(buffer[i] != l[43]->inicio->letra && buffer[i] != l[44]->fim->letra){
                        printf("%c", buffer[i]);
                        i++;
                    }
                }
