...
//void tokens(List **l[]);
void validar_token(List *l[], char palavra);
void add_final(List *l, int elem);
...
/*void tokens(List **l[]){
    List *l;
    int cont_iniciais_iguais = 0, i=0;

    for(i = 0; i < 40; i++){ //vetor de strings
        *l[i] = create_list(); 
        add_final(*l[i], 'a');
        add_final(*l[i], 'u');
        add_final(*l[i], 't');
...
//     while(palavra[i] != '\0'){
//         Node *pont = l[i]->inicio;
//         while(pont != NULL){
//             if(palavra[i] == pont->letras){
//                 cont++;
//                 pont = pont->next;
//             }
//             else{
